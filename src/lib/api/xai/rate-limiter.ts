...
export class RateLimiter {
   ...
   * @param _tokenCount Estimated token count for request
   * @throws {Error} If rate limit would be exceeded
   */
  async checkRateLimit(_tokenCount: number): Promise<void> {
    const now = Date.now();
    const oneMinuteAgo = now - 60000;

    // Clean up old entries
    this.requestTimes = this.requestTimes.filter(_time => _time > oneMinuteAgo);
    this.tokenCounts = this.tokenCounts.filter((_, _i) => this.requestTimes[_i] > oneMinuteAgo);
    ...
    // Check token limit
    const totalTokens = this.tokenCounts.reduce((_sum, _count) => _sum + _count, 0) + _tokenCount;
    ...
    this.tokenCounts.push(_tokenCount);
  }
  ...
  getRateLimitStatus(): { 
    ...
    const activeRequests = this.requestTimes.filter(_time => _time > oneMinuteAgo).length;
    const activeTokens = this.tokenCounts
      .filter((_, _i) => this.requestTimes[_i] > oneMinuteAgo)
